...
private:
	float transposeMatrix[30][30];

public:
	float determinte, cinMatrix[30][30], coutMatrix[30][30];
	int sizeMatrix;

	InverseMatrix(float v[16]){
		int row, col;
	...
	void Cofactor(){
		...
		for (row3=0; row3 < sizeMatrix; row3++){
			for (col3=0; col3 < sizeMatrix; col3++){
				...
				cofactorMatrix[row3][col3] = pow((float)-1, (row3 + col3)) * Determinte(minorMatrix, (sizeMatrix - 1));
			}
		}
	...
	float Determinte(float minorMatrix[30][30], int size){
		...
		if (size == 1){
			...
			for(col=0; col < size; col++){
				...
				sum += (float) (minorMatrix[0][col] * pow((float)-1, col) * Determinte(newMinorMatrix, (size - 1)));	// function
			}
		}
		return (float) sum;
	}// end function
